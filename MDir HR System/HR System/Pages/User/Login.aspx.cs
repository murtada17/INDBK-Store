            ...
            if (!string.IsNullOrEmpty(txtUserName.Text) && !string.IsNullOrEmpty(txtPassword.Text))
            {
                if (txtPassword.Text.Length >= 4)
                {
                    ...
                    cmd.CommandText = @"SELECT        dbo.EmployeesTBL.BranchID, dbo.EmployeesTBL.DepartmentID, dbo.UsersTBL.UserID, dbo.UserPrivilegesTBL.PrivilegeID,
                                                      [FirstNameAR] +' ' + [MidNameAR] + ' ' + [LastNameAR] as FullName
                    ...
                    WHERE        (dbo.UserPrivilegesTBL.ApplicationID = @ApplicationID) AND (dbo.UsersTBL.Password = @Pass) AND (dbo.UsersTBL.UserName = @USER) AND (dbo.UsersTBL.IsActive = 1)";
