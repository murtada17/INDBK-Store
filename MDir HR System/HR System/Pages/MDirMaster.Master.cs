...
namespace HR_Salaries
{
    public partial class MDirMaster : System.Web.UI.MasterPage
    {
        ...
        public static int ApplicationID = 7;  // 30
        //public static int ApplicationID = 3;  // 31 

        //public static int ApplicationID = 1004;
        ...
        //public static SqlConnection con = new SqlConnection("Data Source=DESKTOP-2795K3K\\SQLEXPRESS; Initial Catalog=HR; User ID=HR;Password=1HR12IIB18");
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                SetCmd.CommandText = "SELECT * FROM [HR].[dbo].[SystemSettingsTBL]";
...
<ul class='collapse list-unstyled' id='{1}'>",
        ...
        public static DataTable GetData2(SqlCommand cmd)
        {
            cmd.Connection = con;
            DataTable dt = new DataTable();
            try
        ...
        public static int Execute20(SqlCommand cmd, Label label, string Page)
        {
            bool result;
            int modified = 0;
            try
        ...
        public static bool Execute2(SqlCommand cmd, Label label, string Page)
        {
            bool result;
            try
            {
        ...
        public static Label lbl { get; private set; }
        ...
        public static void ExportExcel(DataTable dt, string FileName)
        {
            ...
            foreach (DataRow dr1 in dt.Rows)
            {
                ...
                for (i = 0; i < dt.Columns.Count; i++)
                {
                    ...
                    HttpContext.Current.Response.Write(tab + dr1[i].ToString());
                    tab = "\t";
                }
                HttpContext.Current.Response.Write("\n");
            }
        ...
        public static string DateConvertion(string Input)
        {

            DateTime dt1 = DateTime.ParseExact(Input, "dd-MM-yyyy hh:mm:ss:tt",
        ...
        public static float getCommission(float Input, Label lbl)
        {

            float Commission = 0;

            SqlCommand cmd_Commission = new SqlCommand();
            cmd_Commission.CommandText = @"
                                       SELECT [id]
                                 ...
                                 FROM [HR].[dbo].[PPC_Process_Rate]
                                            ";
            DataTable dt = MDirMaster.GetData(cmd_Commission, lbl);

            int type_id = int.Parse(dt.Rows[0]["Process_Commission_Type"].ToString());

            if(type_id == 1)
            {
                float Commission_ = float.Parse(dt.Rows[0]["Process_Commission_Value_percentage"].ToString());
                float Commission_Min = float.Parse(dt.Rows[0]["Process_Min_Commission"].ToString());

                float _Commission = Input * Commission_;

                decimal _Commission2 = Convert.ToDecimal(_Commission);
                decimal _Commission3 = Decimal.Truncate(_Commission2);

                if (_Commission < Commission_Min)
                {
                    Commission = Commission_Min;
                }else
                {
                    Commission = (float)_Commission3;
                }
            }
            else if(type_id == 2)
            {
                float Commission_fixed = float.Parse(dt.Rows[0]["Process_Commission_Value_fixed"].ToString());
        ...
        public static string Get_amount(string Input, Label lbl)
        {
           //float comm = MDirMaster.getCommission(float.Parse(Input), lbl);

            SqlCommand cmd_Search_ = new SqlCommand();
            SqlCommand cmd_Search_2 = new SqlCommand();

            cmd_Search_2.CommandText = @"
                                         select sum([Process_Amount]) - sum([Process_Commission]) as P_Amount
                                         FROM [HR].[dbo].[PPC_Processes]
                                         where [From_Account] = @@Input
                                         and [Verification_status] = 1 and Process_Type in (1,2,3)
                                        ";

            cmd_Search_.CommandText = cmd_Search_2.CommandText.Replace("@@Input", Input.ToString());
            DataTable dt = MDirMaster.GetData(cmd_Search_, lbl);
        ...
        public static bool check_Access_process(string Pro_Amount, string DB_Amount)
        {

            decimal Pro_Amount_ = 0;
            ...
            if (decimal.Parse(Pro_Amount) < 0)
            {
                Pro_Amount_ = decimal.Parse(Pro_Amount) * -1;
            }
            else
            {
                Pro_Amount_ = decimal.Parse(Pro_Amount);
            }

            bool status_ = false;

            if (Pro_Amount_ <= decimal.Parse(DB_Amount))
            {
                status_ = true;
            }

            else if (Pro_Amount_ > decimal.Parse(DB_Amount))
            {
                status_ = false;
            }
        ...
        public static bool check_Access_process2(string Pro_Amount, decimal DB_Amount)
        {
            ...
            if (Pro_Amount_ <= DB_Amount)
            {
                status_ = true;
            }

            else if (Pro_Amount_ > DB_Amount)
            {
                status_ = false;
            }
        ...
        public static bool virfecationTXT(TextBox controlss)
        {
            bool TXT_Status = false;

            if (controlss.Text == "" || controlss.Text == null)
            {
                TXT_Status = false;
            }
        ...
        public static bool Get_Account(string Input, Label lbl)
        {
            SqlCommand cmd_Search_ =  new SqlCommand();
            SqlCommand cmd_Search_2 = new SqlCommand();
        ...
        public static bool check_ifAces(int value1, int value2, int value3, int value4, Label lbl)
        {
            bool status_ = false;
  ...
  where [Coll_ContractCode] = '" + value1 + @"' and [Coll_CollateralCode] = '" + value2 + @"'
    and [date_month] = '" + value3 + @"' and [date_year] = '" + value4 + @"'
                                        ";
          
            DataTable dt = MDirMaster.GetData(cmd_Search, lbl);
        ...
        public static bool check_ifAces_COM(int value1, int value3, int value4, Label lbl)
        {
            bool status_ = false;
            SqlCommand cmd_Search = new SqlCommand();

            cmd_Search.CommandText = @"SELSELECT [Com_Comp_RowID]
      ,[Com_ContractCode]
      ,[Com_CustomerCode]
      ,[Com_CompanyName]
      ,[Com_CompanyNameOfficial]
      
  FROM [HR].[dbo].[CBS_Contract_Company]
  where [Com_ContractCode] = '" + value1 + @"' 
    and [date_month] = '" + value3 + @"' and [date_year] = '" + value4 + @"'
                                        ";
        ...
        public static int random_(int min, int max)
        {
            ...
            int numm = rnd.Next(min, max);

            return numm;
        }


        public static string get_name(string account)
        {
            ...
            cmd_Search2.CommandText = @"SELECT
                                        ...
                                        ";
            cmd_Search.CommandText = cmd_Search2.CommandText.Replace("@Account", account);

            DataTable dt;
            string name_ = "";
             dt = MDirMaster.GetData(cmd_Search, lbl);

            try
            {
                name_ = dt.Rows[0]["Account_name"].ToString();
            }
            catch
            {
                name_ = "";
